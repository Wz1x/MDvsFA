...
if __name__ == '__main__':
    train_path = './autodl-tmp/data/MDvsFA_train'
    test_path = './autodl-tmp/data/test'
    img_size = 128
    batch_size = 5
    model_path = './autodl-tmp/output/best_model/20241217_150059'
    is_get_score = True # 是否获得F1分数
    is_get_image = True # 是否获得输出图片
    output_path = './autodl-tmp/output'
    ...
    G1.load_state_dict((torch.load("%s/G1_trade.pth"%(model_path))))
    G2.load_state_dict((torch.load("%s/G2_trade.pth"%(model_path))))
